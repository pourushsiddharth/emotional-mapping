...
def build_prompt(data):
    ...
    prompt = f"""
You are an expert Emotional Mapping AI. Your goal is to create a VERY CONCISE emotional map BASED ON {situation}, {age}, {location} with FEW key stages (3-4 ideally).

THE OUTPUT MUST BE A SINGLE, VALID JSON OBJECT, AND NOTHING ELSE.

Your analysis must identify:
1. Core emotions (lists).
2. Emotional transitions (all).
3. Trigger analysis (all).
4. Psychological interpretation (SEPERATED BY COMMAS AND NOTHING ELSE, NO BRACKETS, lists).
...
6. Suggestions for healing (practical and scientifically proven, lists).
7. A final empathetic message in the specified language (very short).
    ...
    try:
        ...
        if parsed_json:
            ...
            if stages:
                ...
                dot.attr(rankdir='LR',  # <<< CHANGED TO LEFT-TO-RIGHT
                         bgcolor='white', 
                         newrank='true', 
                         splines='spline', 
                         pad="0.2",        
                         nodesep="0.4",    # For LR, this is vertical separation between nodes in same rank/column
                         ranksep="0.5",    # For LR, this is horizontal separation between ranks/columns
                         dpi="72"          
                        )
                ...
                dot.attr('node', 
                         ...
                         margin="0.1,0.08", # Adjust margin: horizontal, vertical
                         fixedsize='true', # Try to enforce width/height
                         height="0.5",     # Node height (in inches) - might need to be taller for LR
                         width="1.0")      # Node width (in inches) - might need to be narrower for LR
                                           # Or, make width slightly more than height for LR text flow
                ...
                dot.attr('edge', 
                         ...
                         fontsize='6',    
                         fontcolor='#616161',
                         minlen="1") # Try to make edges a bit longer if needed for LR clarity
                ...
                for i, stage_data in enumerate(stages):
                    ...
                    if node_id_raw:
                        ...
                        # For LR, if nodes are wider than tall, we might allow slightly longer lines
                        max_chars_per_line = 12 # Adjust based on node width="1.0" and fontsize='8'
                        if len(label_display) > max_chars_per_line : # Basic wrapping for the stage name itself
                            wrapped_label_lines = []
                            current_line = ""
                            ...
                            label_display = r"\n".join(wrapped_label_lines[:2]) # Max 2 lines for visible label
